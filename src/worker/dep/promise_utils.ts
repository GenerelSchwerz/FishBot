 ...
 */
export function onceWithCleanup(emitter: EventEmitter, event: string, { timeout = 0, checkCondition = undefined }: { timeout?: number, checkCondition?: Function } = {}) {
...
export function withTimeout(promise: any, timeout: number) {
    return Promise.race([
        promise,
        sleep(timeout).then(() => {
